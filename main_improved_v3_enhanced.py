...
from improved_model_v3_enhanced import ImprovedBotnetDetectorV3Enhanced, ImprovedTrainerV3Enhanced


def parse_args():
    parser = argparse.ArgumentParser(description='V3 Enhanced - 增加分离度')
    ...
    parser.add_argument('--model_path', type=str, default='improved_botnet_model_v3_enhanced.pth')
...
def train_model(args, device):
    """训练 V3 Enhanced 模型"""
    print("\n" + "="*60)
    print("开始训练 V3 Enhanced 僵尸网络检测模型")
    print("核心改进：Focal Loss + Margin Contrastive Loss 增加分离度")
    print("="*60)
    ...
    model = ImprovedBotnetDetectorV3Enhanced(
        stat_dim=stat_dim,
    ...
    print(f"  - 损失函数：Focal Loss + Margin Contrastive Loss")
    
    trainer = ImprovedTrainerV3Enhanced(model, lr=args.lr, device=device)
...
def evaluate_model(args, device, verbose=True):
    ...
    if verbose:
        print("\n[Phase 3] 加载模型...")
    model = ImprovedBotnetDetectorV3Enhanced.load(args.model_path, device=device)
    model.eval()
...
def main():
    ...
    if result:
        ...
        np.savez(
            'v3_enhanced_score_distribution.npz',
            probs=probs,
            y_true=y_true,
            normal_probs=probs[y_true == 0],
            bot_probs=probs[y_true == 1]
        ...
        with open('v3_enhanced_results.json', 'w') as f:
            json.dump(json_results, f, indent=2)
        print(f"\n[Info] 结果已保存到 v3_enhanced_results.json 和 v3_enhanced_score_distribution.npz")
