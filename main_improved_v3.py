...
from improved_heterogeneous_graph_v3 import ImprovedHeterogeneousGraphBuilderV3, MiniBatchGraphBuilderV3
from improved_model_v3 import ImprovedBotnetDetectorV3, ImprovedTrainerV3


def parse_args():
    parser = argparse.ArgumentParser(description='改进的僵尸网络检测 V3 - 速度优化 + 修复数据泄露')
    ...
    parser.add_argument('--model_path', type=str, default='improved_botnet_model_v3.pth')
...
def train_model(args, device):
    """训练 V3 模型"""
    print("\n" + "="*60)
    print("开始训练 V3 改进的僵尸网络检测模型")
    print("核心改进：增强分数区分度 + 速度优化 + 修复数据泄露")
    print("="*60)
    ...
    # 构建图 - 使用 V3 快速版本
    print("\n[Phase 2] 构建三模态异构图 (快速版)...")
    builder = ImprovedHeterogeneousGraphBuilderV3()
    data, ip_map, norm_stats = builder.build(
        df, 
        include_semantic=not args.no_semantic,
        include_struct=not args.no_struct,
        fit_stats=None  # 训练集，计算统计量
    )
    ...
    # 准备训练数据 - 使用简单随机采样
    print("\n[Phase 3] 准备训练数据 (简单随机采样)...")
    MiniBatchGraphBuilderV3.prepare_for_training(data, labels, train_ratio=0.8)
    ...
    model = ImprovedBotnetDetectorV3(
        stat_dim=stat_dim,
    ...
    trainer = ImprovedTrainerV3(model, lr=args.lr, device=device)
    ...
    # 保存归一化统计量（用于测试集）
    import pickle
    with open('norm_stats_v3.pkl', 'wb') as f:
        pickle.dump(norm_stats, f)
    print(f"[Info] 归一化统计量已保存至 norm_stats_v3.pkl")
...
def evaluate_model(args, device, verbose=True):
    ...
    # 加载归一化统计量
    norm_stats_path = 'norm_stats_v3.pkl'
    if os.path.exists(norm_stats_path):
        import pickle
        with open(norm_stats_path, 'rb') as f:
            norm_stats = pickle.load(f)
        if verbose:
            print(f"[Info] 已加载归一化统计量")
    else:
        norm_stats = None
        if verbose:
            print("[Warn] 未找到归一化统计量，将重新计算")
    
    # 构建图 - 使用训练集的统计量
    if verbose:
        print("\n[Phase 2] 构建测试图 (快速版)...")
    builder = ImprovedHeterogeneousGraphBuilderV3()
    data, ip_map, _ = builder.build(
        df,
        include_semantic=not args.no_semantic,
        include_struct=not args.no_struct,
    ...
    if verbose:
        print("\n[Phase 3] 加载模型...")
    model = ImprovedBotnetDetectorV3.load(args.model_path, device=device)
    model.eval()
...
def main():
    ...
    if result:
        ...
        print(f"\n[Info] 结果已保存到 v3_results.json 和 v3_score_distribution.npz")
