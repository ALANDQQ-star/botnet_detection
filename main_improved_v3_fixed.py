...
def parse_args():
    parser = argparse.ArgumentParser(description='修复版V3 - 使用V2特征+优化训练')
    ...
    parser.add_argument('--model_path', type=str, default='improved_botnet_model_v3_fixed.pth')
...
def train_model(args, device):
    """训练修复版V3模型"""
    print("\n" + "="*60)
    print("开始训练 修复版V3 僵尸网络检测模型")
    print("使用V2特征提取器 + V3优化训练流程")
    print("="*60)
    ...
    # 构建图 - 使用V2的图构建器（原始特征提取器）
    print("\n[Phase 2] 构建三模态异构图（使用V2特征提取器）...")
...
def main():
    ...
    if result:
        ...
        np.savez(
            'v3_fixed_score_distribution.npz',
            probs=probs,
            y_true=y_true,
            normal_probs=probs[y_true == 0],
            bot_probs=probs[y_true == 1]
        ...
        print(f"\n[Info] 结果已保存到 v3_fixed_results.json 和 v3_fixed_score_distribution.npz")
